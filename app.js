...
const novafunc = () => {
   ...
   const squareRoot = unos * unos
   console.log(`Kvadrat broja ${unos} , je ${squareRoot}`)
};
...
// Corrent console log should be: The pow of 5 is 25
...
const NOVI_URL = "https://reqres.in/api"

const uzmiUzere = async () => {

    const uzmiUsere = await fetch(`${NOVI_URL}/users?page=2`);

    const noviUseri = uzmiUsere.json();

    return noviUseri;
}

uzmiUzere().then((res) => console.log(res.data))
